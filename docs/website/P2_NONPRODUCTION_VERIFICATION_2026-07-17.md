...
still a stub rather than an active adapter. Founder authentication opened Talpro
portal `244059903`. Read-only inspection found 6,079 open tasks, current real
contact/email activity, only one visible account and no sandbox label or
`Sandboxes` Settings entry. The portal therefore fails the non-production gate
and is explicitly excluded from P2 test writes. Codex made no provider write and
did not touch OAuth, credentials, tokens or provider settings. Full evidence and
the fail-closed resume conditions are recorded in
`docs/website/P2_CRM_SANDBOX_CONNECTION_2026-07-18.md`.
